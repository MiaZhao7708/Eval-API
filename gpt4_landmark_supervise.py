...
INPUT_FILE = f'{MYPATH}/open-source/LLaVA-Finetune/world_knowledge/google-v2/landmark_mid25k_supervise_2k_tonote_label.json'
...
def process_item(item, history):
    ...
    prompt = [
        { 
            "type": "text",
            "text": prompt_landmark.get_prompt_supervise(item)
        },
        { 
            "type": "image_url", 
            "image_url": {
...
def run_multithread_processing(threads):
    ...
    cache_file = os.path.dirname(INPUT_FILE) + '/output_gpt4_2410_tonote_2k_rankmid_supervise.jsonl'
...
def test(item):
    ...
    prompt = [
        { 
            "type": "text",
            "text": prompt_landmark.get_prompt(item['label'])
        },
        { 
            "type": "image_url", 
            "image_url": {
